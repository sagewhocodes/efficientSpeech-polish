...
def get_yaml_path(config_dir,name):
  return os.path.join(config_dir, name+'.yaml')


def get_yaml_contents(config_dir,name):
  with open(get_yaml_path(config_dir,name), 'r') as f:
    return yaml.safe_load(f.read())
            

def write_yaml(config_dir,name, contents):
  with open(get_yaml_path(config_dir,name), 'w') as f:
    f.write(yaml.dump(contents))


def write_to_file(wavs, preprocess_config,epoch, lengths=None, wav_path="outputs", filename="tts"):
    wavs = (
    ...
    wav_path = wav_path + "/" +  f"epoch_{epoch}"
    ...
    if len(wavs) == 1:
        ...
        for i, wav in enumerate(wavs):
            path = os.path.join(wav_path, "{}-{}-epoch{}.wav".format(filename, i+1,epoch))
...
def get_args():
    ...
    parser.add_argument("--preprocess-config",
                        default="config/MLS_Polish/preprocess.yaml",
                        type=str,

...
from experiments.final_experiment.ef-polish.model import EfficientSpeech
...
def tts(preprocess_config, model, is_onnx, args):
    text = args.text.strip()
    text = text.lower()
    text = text.replace('-', ' ')
    phoneme = np.array([text2phoneme(text)], dtype=np.int32)
    ...
    if is_onnx:
        ...
        phoneme = np.array(
            [text2phoneme(text)], dtype=np.int32)

...
class LJSpeechDataModule(LightningDataModule):
    ...
    def collate_fn(self, batch):
        ...
        phoneme_lens = torch.from_numpy(np.array([phoneme.shape[0] for phoneme in phonemes]))
        max_phoneme_len = torch.max(phoneme_lens).item()
        pitches = pad_1D(pitches,maxlen=max_phoneme_len)
        energies = pad_1D(energies,maxlen=max_phoneme_len)
        durations = pad_1D(durations,maxlen=max_phoneme_len)
        mels = pad_2D(mels)
        ...
        # print(f"phoneme mask: {phoneme_mask.shape}")
        # print("*"*60)
...
class LJSpeechDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        phoneme = np.array(text_to_sequence(self.text[idx]))
